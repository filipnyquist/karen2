import { pgTable, uuid, varchar, text, timestamp, integer, boolean, pgEnum, uniqueIndex, } from "drizzle-orm/pg-core";
...
export const userEducations = pgTable("user_educations", {
    ...
    userEducationIdx: uniqueIndex("user_education_idx").on(table.userId, table.educationType),
}));
// Locations
...
export const eventWorkers = pgTable("event_workers", {
    id: uuid("id").defaultRandom().primaryKey(),
...
export const skipQueueTickets = pgTable("skip_queue_tickets", {
    ...
    redeemedAtEventId: uuid("redeemed_at_event_id").references(() => events.id, { onDelete: "set null" }),
...
export const skipQueueTicketsRelations = relations(skipQueueTickets, ({ one }) => ({
    user: one(users, {
